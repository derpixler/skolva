...
tp2_unit() {
    ...
    step "1.3" "Auth — CheckPermission service (admin bypass)"
    run_go_test "CheckPermission" ./internal/auth/ -v -run "TestServiceCheckPermission"

    step "1.4" "OpenAPI — spec validation + route parity"
    run_go_test "openapi" ./internal/app/ -v -run "TestOpenAPI"

    # Core unit tests (middleware, mail, secrets, metadata, search, config,
    # types, errors) moved to the skolva-core repo and run in its own CI.
}
